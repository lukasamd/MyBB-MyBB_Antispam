...
var Thread = {
	...
	quickReplyDone: function(request)
	{
        if($('mybbAntispam_img'))
        {   
            mybbAntispamData = request.responseText.match(/^<captcha>([0-9a-zA-Z]+)\|([0-9a-zA-Z]+)\|([0-9]+)\|([0-9a-zA-Z]+)\|([0-9a-zA-Z]+)<\/captcha>/);
            if(mybbAntispamData)
            {
                request.responseText = request.responseText.replace(/^<captcha>(.*)<\/captcha>/, '');
                
                if(mybbAntispamData[1])
                {
                    
                    var oldHash = $('specialHash').value;
                    $('imagehash').value = mybbAntispamData[4];
                    
                    if(mybbAntispamData[5] != '0')
                    {
                        $(oldHash).type = "hidden";
                        $(oldHash).value = mybbAntispamData[5];
                        $('captcha_trow').style.display = "none";
                    }
                    else
                    {
                        $('mybbAntispam_img').src = "captcha.php?action=regimage&imagehash=" + mybbAntispamData[4];
                        $(oldHash).type = "text";
                        $(oldHash).value = "";
                        $('captcha_trow').style.display = "";
                    }
                    $(oldHash).setAttribute("name", mybbAntispamData[2]);
                    $(oldHash).setAttribute("id", mybbAntispamData[2]);
                    $('specialHash').value = mybbAntispamData[2];
                }
            }
        }
